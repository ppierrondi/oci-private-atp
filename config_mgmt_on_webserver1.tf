resource "null_resource" "paulopierrondiWebserver1_ConfigMgmt" {
  depends_on = [oci_core_instance.paulopierrondiWebserver1, oci_database_autonomous_database.paulopierrondiATPdatabase, local_file.paulopierrondi_ATP_database_wallet_file]

  provisioner "remote-exec" {
    connection {
      type        = "ssh"
      user        = "opc"
      host        = data.oci_core_vnic.paulopierrondiWebserver1_VNIC1.public_ip_address
  ...
  provisioner "file" {
    ...
    source      = var.paulopierrondi_ATP_tde_wallet_zip_file
    destination = "/tmp/${var.paulopierrondi_ATP_tde_wallet_zip_file}"
  }
  ...
  provisioner "remote-exec" {
    ...
    inline = ["echo '== 4. Unzip TDE wallet zip file'",
      "sudo -u root unzip -o /tmp/${var.paulopierrondi_ATP_tde_wallet_zip_file} -d /usr/lib/oracle/18.3/client64/lib/network/admin/",
...
resource "null_resource" "paulopierrondiWebserver1_Flask_WebServer_and_access_ATP" {
  depends_on = [null_resource.paulopierrondiWebserver1_ConfigMgmt]
